...
export default function Header({ isWhiteBackground}) {
  const [isModalOpen, setIsModalOpen] = useState(false)
  const toggleModal = () => setIsModalOpen(!isModalOpen)

  return (
    <header className={`fixed top-0 w-full p-16 z-50 flex justify-between items-center transition duration-300 ${isWhiteBackground ? "text-black" : "text-white"}`}>
      <Link href='/'>
        <div className='font-homemade text-5xl'>mitsch</div>
      </Link>

      {/* Burger Menu Icon */}
      <button className='focus:outline-none text-white' onClick={toggleModal} aria-label='Open Menu'>
        {/* Burger icon */}
      ...
      {/* Modal for Full-Screen Navigation */}
      {isModalOpen && (
        <div className='fixed inset-0 bg-black z-50 flex flex-col justify-center items-center'>
          ...
          {/* Navigation Items */}
          <nav className='font-cooperhewitt mt-16 flex flex-col space-y-8 text-center'>
